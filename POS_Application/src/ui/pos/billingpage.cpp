...
void BillingPage::on_createReceipt_button_clicked() {
...
void BillingPage::on_cancelReceipt_button_clicked() {
  if (this->model.getPageAccess(2) == User::PageAccess::EDITABLE) {
...
void BillingPage::on_payReceipt_button_clicked() {
  if (this->model.getPageAccess(2) == User::PageAccess::EDITABLE) {
    ...
    if (this->receipStack) {
      ...
      qDebug() << "precio del recibo: " << receipt->getReceiptPrice();
      ProcessReceiptDialog processReceiptDialog(this
          , receipt->getReceiptPrice());
      
      if (processReceiptDialog.exec() == QDialog::Accepted) {
        ...
        this->connect(&previewDialog, &QPrintPreviewDialog::paintRequested
            , this, &BillingPage::printPreview);
...
void BillingPage::printPreview(QPrinter* printer) {
...
void BillingPage::createSelectProductButtons(size_t numberOfProducts) {
  ...
  for (const auto& product : products) {
    ...
    this->connect(button
        ...
        , this, &BillingPage::addProductToReceipt);
...
void BillingPage::addProductToReceipt(const Product& product) {
  if (this->model.getPageAccess(2) == User::PageAccess::EDITABLE) {
