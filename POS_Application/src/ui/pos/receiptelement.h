...
/**
 * @class ReceiptElement
 * @brief Represents a single item within a receipt.
 *
 * The ReceiptElement widget displays the product information (name, price, and quantity)
...
private:
  Ui::ReceiptElement* ui = nullptr; ///< Pointer to the UI elements.
  const Product& product;           ///< Reference to the associated Product.
  size_t quantity = 1;              ///< Current quantity of the product in the receipt.
  double totalPrice = 0;            ///< Total price calculated as quantity * product price.
  
public:
  /**
   * @brief Constructs a ReceiptElement widget.
   *
   * Initializes the ReceiptElement with the given product and sets up the display
...
public:
  /**
   * @brief Generates the HTML content for this receipt element.
   *
   * Returns an HTML-formatted string that contains the product name, quantity,
   * unit price, and the total price for this receipt element.
   *
   * @return QString containing the HTML representation of the receipt element.
   */
  QString content();
